pop = function(){
	...
	function pop(p){
		this.width   = p.width || 300;//瀹藉害
		this.height  = p.height || 300;//楂樺害
		this.title   = p.title || 'test';//鏍囬
		this.content = p.content || 'test';//鍐呭锛堝彲鍖呮嫭html锛�
		this.hasbg   = p.hasbg || false;//鏄惁浣跨敤鑳屾櫙锛堥粯璁や笉浣跨敤锛�
		this.pos     = p.pos || 'mid';//浣嶇疆锛堥粯璁ゅ眳涓級
		this.bgColor = p.bgColor || 'E6E6E6';//鑳屾櫙棰滆壊
		this.borColor= p.borColor || 'fff';//杈规棰滆壊
		this.borWidth= p.borWidth || 5;//杈规瀹藉害
		this.drag    = p.drag || false;//鏄惁鎷栧姩
		this.closePos= p.closePos || 'right';//鍏抽棴鎸夐挳浣嶇疆
